...
class VulnScannerOrchestrator:
    ...
    def save_report(self):
        ...
        md_path = write_markdown(str(Path(self.output_dir) / f"{base}.md"), md)
        html_body = self._build_html()
        html_path = write_html(str(Path(self.output_dir) / f"{base}.html"), f"Vulnerability Scan - {self.target}", html_body)
